...
<div class="postbox optionbox">
	<h3><label for="title">Usage Details</label></h3>
	<div class="inside"><H4>Step 1:</H4>
		<p>Enter a name and the full URL (with http://) for each of your feeds. The name will be used to identify which feed produced the link (see the Attribution Label option below). Click Save Settings.</p>
		
		<H4>Step 2 (how to present the feeds):</H4><p>First, decide whether you want to have the feeds go into blog posts, so people can comment on them.  If so, you can go directly to the Feed to Post Options, set them and you are done.</p>
		
		<p>If you'd rather have the feeds show up in a more typical way (and use templates to customize how they look), go to the tab called <a href="/wp-admin/options-general.php?page=wp_rss_multi_importer_admin&tab=setting_options">Setting Options</a>, choose options and click Save Settings.</p>
		
		<H4>Step 3 (only relevant if you're not using the Feed to Post feature):</H4><p>Put this shortcode, [wp_rss_multi_importer], on the page you wish to have the feed.</p>
...
function wp_rss_multi_importer_more_page(){
   ?>	
	...
	<div id="poststuff">
		...
		<div class="postbox">
			<h3><label for="title">How Does This Work?</label></h3>
			<div class="inside">
<p>This plugin does several different things.  Mainly, it brings in RSS feeds and aggregates and sorts them and puts them on your web site.  You can assign categories to the feeds.</p>
<p>It can put them on your web site in 3 different ways. First, it can present them using shortcode using one of 8 different templates (see the Settings Option for this).  Second, it can take the feeds and put them directly into your blog posts, so people can comment on them (see the Feed to Post Options for this).  Third, it provide you with a feed to give your readers (see the Export Feed Options for this).</p>
<p>Finally, you don't have to choose one way or another to present the feeds.  You can do all 3 at the same time.</p>


</div></div>
<div class="postbox">
	<h3><label for="title">Help Us Help You</label></h3>
	<div class="inside">
